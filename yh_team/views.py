...
from yh_team.forms import TeamForm
...
def index(request):
    ...
    if 'keyword' in request.GET:
        ...
        team_items = Team.objects.filter(libelle__icontains=keyword)  # like %keyword%
    else:
        team_items = Team.objects.all()  # get all level studys items
        keyword = ""  # set default value for keyword

    paginator = Paginator(team_items, 5)  # Show 5 level studys per page
...
def update(request, id):
    ...
    if form.is_valid():
        ...
        messages.info(request, 'team updated')  # display a message to the user
        return redirect('team.index')  # redirect to the level study index
...
def delete(request, id):
    ...
    if request.method == 'POST':
        ...
        messages.info(request, 'Team deleted')  # display success message to the user
        return redirect('team.index')  # redirect to the level study index
